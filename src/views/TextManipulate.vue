...
**${item['polnoe-naimenovanie-tovara-dlya-klienta']}**${item['polnoe-naimenovanie-tovara-dlya-klienta'].toString().toLowerCase().includes('фальшпанель') ? ' — декоративная панель для отделки кухонного гарнитура.' : ''}${item['polnoe-naimenovanie-tovara-dlya-klienta'].toString().toLowerCase().includes('дверь') ? ' защищает содержимое шкафов от пыли, задает внешний вид кухонного гарнитура.' : ''}${item['polnoe-naimenovanie-tovara-dlya-klienta'].toString().toLowerCase().includes('угол') ? ' — важная составная часть кухонного гарнитура.' : ''} ${insertProp(item['samostoyatelnaya-sborka'])} Основной материал - ${insertProp(item['osnovnoj-material'])}. ${insertProp(item['sootvetstvie-vnutrennej-storone'])} ${insertProp(item['predvaritelnoe-sverlenie-fasada'])}
